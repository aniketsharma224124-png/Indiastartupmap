  ...
  return (
    <>
      ...
      <div className="flex flex-col overflow-hidden"
        style={{ background: '#0B0B1A', border: '1px solid rgba(155,111,255,0.18)', borderRadius: '16px', height: '100%', animation: 'panelIn 0.22s ease' }}>
    ...
    <div className="card flex flex-col h-full" style={{ minHeight: '520px' }}>
      ...
      {view === 'form' ? (
        ...
        <div className="flex flex-col flex-1 p-4 pt-3">
          ...
          {/* Startup list */}
          <div className="flex-1 overflow-y-auto space-y-3" style={{ scrollbarWidth: 'thin', scrollbarColor: 'rgba(74,158,255,0.2) transparent' }}>
            {loading ? (
              <div className="text-center py-12 text-white/25 text-sm">Loading startups…</div>
...
export default function Home() {
  ...
  const handleProceedToPayment = async () => {
    ...
    const packAmounts = { credits_10: 99900, credits_25: 199900, credits_50: 349900 }
    const packCredits = { credits_10: 10, credits_25: 25, credits_50: 50 }
    ...
    const options = {
      key: keyId,
      amount: packAmounts[selectedPack.id],
      currency: 'INR',
      name: 'IndiaStartupMap',
      description: `${selectedPack.label} Intro Credits`,
      handler: async (response) => {
        const { doc, updateDoc, increment, getFirestore } = await import('firebase/firestore')
        const db = getFirestore()
        await updateDoc(doc(db, 'users', user.uid), {
          intro_credits: increment(packCredits[selectedPack.id])
        })
        toast.success(`${packCredits[selectedPack.id]} intro credits added to your account!`)
    ...
    <main className="relative z-10 pt-20">
      ...
      <section className="max-w-7xl mx-auto px-6 pt-10 pb-6 text-center relative">
        ...
        <h1 className="text-4xl md:text-5xl lg:text-6xl font-black leading-[1.08] mb-4"
          ...
          Discover Startups and Investors<br />Across India
        </h1>
      ...
      <section className="max-w-7xl mx-auto px-6 pb-10" id="list-startup">
        <div className="grid grid-cols-1 lg:grid-cols-[55%_43%] gap-5 items-stretch">
          ...
          <div className="flex flex-col justify-between h-full">
            ...
            {/* Buy More Intro Credits — below map, same column */}
            {mode === 'startup' && (
              <div className="mt-5 rounded-xl p-4 flex-shrink-0" style={{ background: 'rgba(74,158,255,0.05)', border: '1px solid rgba(74,158,255,0.15)' }}>
                <div className="text-[10px] font-black tracking-[2px] uppercase text-blue-400 mb-3">✉️ Buy More Intro Credits</div>
                <p className="text-xs text-white/35 mb-4">Intro credits let you pitch directly to investors. Select a pack below.</p>
                <div className="flex flex-col gap-2 mb-4">
                  {[
                    { label: '10 Intros', price: '₹999', id: 'credits_10' },
                    { label: '25 Intros', price: '₹1,999', id: 'credits_25', popular: true },
                    { label: '50 Intros', price: '₹3,499', id: 'credits_50' },
                  ].map(pack => (
                    <button
                      key={pack.id}
                      onClick={() => handleBuyCredits(pack)}
                      className="w-full flex items-center justify-between px-4 py-3 rounded-xl border transition-all"
                      style={selectedPack?.id === pack.id
                        ? { background: 'rgba(74,158,255,0.15)', borderColor: 'rgba(74,158,255,0.5)', color: '#fff' }
                        : { background: 'rgba(255,255,255,0.03)', borderColor: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.6)' }
                      }
                    >
                      <div className="flex items-center gap-2">
                        <span className="font-black text-sm">{pack.label}</span>
                        {pack.popular && <span className="text-[9px] font-black px-2 py-0.5 rounded-full" style={{ background: 'rgba(246,201,14,0.15)', color: '#F6C90E', border: '1px solid rgba(246,201,14,0.3)' }}>BEST VALUE</span>}
                      </div>
                      <span className="font-black text-sm" style={{ color: '#4A9EFF' }}>{pack.price}</span>
                    </button>
                  ))}
                </div>
                <button
                  ...
                  style={{ background: selectedPack ? 'linear-gradient(135deg,#1a4a8a,#2a6abf)' : 'rgba(255,255,255,0.05)', opacity: selectedPack ? 1 : 0.5, cursor: selectedPack ? 'pointer' : 'not-allowed' }}
                >
                  {selectedPack ? `Buy ${selectedPack.label} for ${selectedPack.price} →` : 'Select a pack to continue'}
                </button>
                {!user && <p className="text-[10px] text-white/25 text-center mt-2">Login required to purchase credits</p>}
              </div>
            )}
          </div>
      ...
      <footer className="border-t border-white/[0.06] px-6 py-8">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="text-sm text-white/25">© 2026 IndiaStartupMap · Made with ❤️ in India 🇮🇳</div>
